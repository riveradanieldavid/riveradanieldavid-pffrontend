...
// LOCAL
// const baseUrl = 'http://localhost:9090/api/abouts';
...
export class AboutService {
    ...
    getAll(): Observable<About[]> {
        return this.http.get<About[]>(baseUrl);
    }

    get(id: any): Observable<About> {
        return this.http.get<About>(`${baseUrl}/${id}`);
    }
    ...
    findByTitle(title: any): Observable<About[]> {
        return this.http.get<About[]>(`${baseUrl}?title=${title}`);
    }
}
